 ...
 * Example: Given Type = string | undefined, the NonOptionalFOrmControlValue = FormControl<string | null>
 */
type NonOptionalFormControl<Type> = FormControl<NoUndefined<Type> | null>;
type NonOptionalNorNullableFormControl<Type> = FormControl<NoUndefined<Type>>;
...
export type FormControlsFor<T> = {
  [K in keyof T]: T[K] extends any[]
    ...
    ? FormGroup<FormControlsFor<Required<Exclude<T[K], undefined>>>>
    : NonOptionalFormControl<T[K]>; // string boolean number
};

export type NonNullableFormControlsFor<T> = {
  [K in keyof T]: T[K] extends any[]
    ? NonOptionalNorNullableFormControl<T[K]>
    : T[K] extends Date
    ? NonOptionalNorNullableFormControl<Date>
    : T[K] extends null
    ? NonOptionalNorNullableFormControl<T[K]>
    : T[K] extends Record<any, any> | undefined
...
type ToFlatStrictFormArray<Type, ArrayElementType> = ArrayElementType extends Date
  ? NonOptionalFormControl<Date>
...
export type TypedFormGroup<Type> = BasicFormGroup<Type> | SimpleFormGroup<Type> | StrictFormGroup<Type> | FlatFormGroup<Type>;

export type TypedAbstractControl<T> = TypedFormGroup<T> | FormControl<T>;

export class SimpleFormGroup<Type> extends FormGroup<SimpleFormControlsFor<Type>> {}
...
export class UnrestrictedFormGroup<Type> extends FormGroup<{
  [Key in keyof Type]-?: Type[Key] extends (infer ArrayElementType)[]
    ? TypedFormArray<ArrayElementType> | NonOptionalFormArray<ArrayElementType> | NonOptionalFormControl<ArrayElementType[]>
